...
def resize_attn_map(attentions, h, w, align_corners=False):
    ...
    assert attentions.shape[2] == h_featmap * w_featmap, f'{attentions.shape[2]} = {h_featmap} x {w_featmap}, h={h}, w={w}'

    bs = attentions.shape[0]
    nh = attentions.shape[1]  # number of head
    groups = attentions.shape[3]  # number of group token
    # [bs, nh, h*w, groups] -> [bs*nh, groups, h, w]
    attentions = rearrange(attentions, 'bs nh (h w) c -> (bs nh) c h w', bs=bs, nh=nh, h=h_featmap, w=w_featmap, c=groups)
...
class ViTSegInference(EncoderDecoder):
    ...
    def __init__(self, model, text_embedding, with_bg, test_cfg=dict(mode='whole', bg_thresh=.95)):
        ...
        logger = get_logger()
        logger.info(f'Building ViTSegInference with {self.num_classes} classes, test_cfg={test_cfg}, with_bg={with_bg}')
    ...
    def get_attn_maps(self, img, return_onehot=False, rescale=False):
        ...
        image_features, patch_features, mid_states = self.model.clip.encode_image(img, return_hidden=True)
        attns_list = mid_states['attns']
        for att_ in attns_list:
            results['attn_dicts'].append({'soft': att_['soft_attn'].unsqueeze(1)})
        ...
        for i in range(len(attn_maps)):
            ...
            if rescale:
                ...
                attn_map = F.interpolate(attn_map, size=img.shape[2:], mode='bilinear', align_corners=self.align_corners)
    ...
    def encode_decode(self, img, img_metas):
        """Encode images with backbone and decode into a semantic seg_segmentation
        ...
        img_outs = {}
        image_features, patch_features, mid_states = self.model.clip.encode_image(img, return_hidden=True)
        img_outs['image_feat'] = patch_features[:, 1:, :].contiguous()
        img_outs['image_x'] = image_features
        ...
        affinity_mask.scatter_add_(dim=-1, index=avg_affinity_topk.indices, src=torch.ones_like(avg_affinity_topk.values))
    ...
    def show_result(self, img_show, img_tensor, result, out_file, vis_mode='input'):
        ...
        assert vis_mode in ['input', 'pred', 'input_pred', 'all_groups', 'first_group', 'final_group', 'input_pred_label'], vis_mode
        ...
        if vis_mode == 'input':
            ...
            self.blend_result(img=img_show, result=result, out_file=out_file, opacity=0.8, with_bg=self.with_bg)
            ...
            blended_img = self.blend_result(
                img=img_show, result=result, out_file=None, opacity=0.6, with_bg=self.with_bg)
            ...
            for layer_idx, attn_map in enumerate(attn_map_list):
                ...
                palette_ = GROUP_PALETTE[sum(num_groups[:layer_idx]):sum(num_groups[:layer_idx + 1])]
                self.blend_result(
                    img=img_show,
                    result=group_result,
                    palette=palette_,
